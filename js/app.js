	...
	(() => {
			...
			if (document.querySelector(".icon-menu")) document.addEventListener("click", (function (e) {
				if (bodyLockStatus && e.target.closest(".icon-menu")) {
					...
					if (document.documentElement.classList.contains("catalog-open")) {
						...
						if (document.querySelector("._sub-menu-open")) document.querySelector("._sub-menu-open").classList.remove("_sub-menu-open");
					}
				}
			}));
		}
		...
		function initSliders() {
			...
			if (document.querySelector(".products-slider__swiper")) {
				let productsSliders = document.querySelectorAll(".products-slider__swiper");
			...
			if (document.querySelector(".new-products__swiper")) {
				const newSliders = document.querySelectorAll(".new-products__swiper");
				newSliders.forEach((element => {
					new core(element, {
						modules: [Navigation, Pagination, Parallax, Autoplay],
						slidesPerView: 3,
						spaceBetween: 30,
			...
			if (document.querySelector(".images-product")) {
				let productThumbs = new core(".thumbs-images__swiper", {
...
***************************************************************************** */
		...
		function rangeInit() {
			const rangeItems = document.querySelectorAll("[data-rangem]");
			if (rangeItems.length) rangeItems.forEach((function (item) {
				let rangeItem = item.querySelector("[data-range-item]");
				let valueFrom = item.querySelector(".price-filter__input_from");
				let valueTo = item.querySelector(".price-filter__input_to");
				nouislider.create(rangeItem, {
					start: [Number(valueFrom.value), Number(valueTo.value)],
					connect: true,
					tooltips: [true, true],
					step: 100,
					range: {
						min: [0],
						max: [Number(rangeItem.dataset.maxValue)]
					},
					format: wNumb({
						decimals: 0
					})
				...
				rangeItem.noUiSlider.on("update", (function (values, handle) {
					...
					if (handle) valueTo.value = Math.round(Number(value)); else valueFrom.value = Math.round(Number(value));
				}));
				valueFrom.addEventListener("change", (function () {
					rangeItem.noUiSlider.set([this.value, null]);
				}));
				valueTo.addEventListener("change", (function () {
					rangeItem.noUiSlider.set([null, this.value]);
				}));
			}));
		}
		rangeInit();
		...
		if (menuBlocks.length) menuBlocks.forEach((function (menuBlock) {
			const menuBlocksItems = menuBlock.querySelectorAll(".sub-menu-catalog__category");
			menuBlock.classList.add(`sub-menu-catalog__block_${menuBlocksItems.length}`);
		}));
		function documentActions(e) {
			const targetElement = e.target;
			if (e.target.hasAttribute("data-parent")) {
				e.preventDefault();
				const subMenuId = targetElement.dataset.parent ? targetElement.dataset.parent : console.log("");
				const subMenu = document.querySelector(`[data-menu="${subMenuId}"]`);
				let allSubmenu = document.querySelectorAll("[data-menu]");
				let allParentsSubmenu = document.querySelectorAll("[data-parent]");
				...
				if (targetElement.classList.contains("_sub-menu-active")) {
					...
					allSubmenu.forEach((function (item) {
						item.classList.remove("_sub-menu-open");
					}));
					allParentsSubmenu.forEach((function (item) {
						item.classList.remove("_sub-menu-active");
					}));
					catalogMenu.classList.remove("_sub-menu-show");
				} else if (!targetElement.classList.contains("_sub-menu-active")) {
					allSubmenu.forEach((function (item) {
						item.classList.remove("_sub-menu-open");
					}));
					allParentsSubmenu.forEach((function (item) {
					...
					catalogMenu.classList.add("_sub-menu-show");
				}
			}
			if (e.target.classList.contains("menu-top-header__link_catalog")) {
				e.preventDefault();
				document.documentElement.classList.add("catalog-open");
			}
			if (e.target.classList.contains("menu-catalog__back")) {
				e.preventDefault();
				if (document.documentElement.classList.contains("catalog-open")) document.documentElement.classList.remove("catalog-open");
			}
			if (e.target.classList.contains("sub-menu-catalog__back")) {
				e.preventDefault();
				if (document.querySelector("._sub-menu-open")) {
					document.querySelector("._sub-menu-open").classList.remove("_sub-menu-open");
					if (document.querySelector("._sub-menu-show")) document.querySelector("._sub-menu-active").classList.remove("_sub-menu-active");
				}
			}
		}
		window["FLS"] = true;
